...
<script>
    // component name
    let compname="SetFileInterfaceStore";
    ...
    const setStore = () => {
        ...
        setfistore=call_aurora("setGroupFileInterfaceStore",params);
    };
    ...
    async function getData () {    
        ...
        if (getmd.err == 0) {         
            // save its name
            name = getmd.metadata[MD["fi_store"]]||"";
            // also update newname        
            newname = getmd.metadata[MD["fi_store"]]||"";
...
{#if !disabled}
    {#if data != undefined}
        ...
        {/await}
    {/if}
    {#if setfistore != undefined}
        {#await setfistore}
            <Status message="Updating FileInterface store on GROUP with id {id} to {newname}..." type="processing" />
        {:then result}
            {#if result.err == 0}
                {setFIStore()}
            {/if}
    ...
    {#if show}    
        ...
        <div class="ui_title ui_center">Edit FileInterface Store</div>              
        <Table data={mdtabledata} headers={mdtable} orderdirection={0} />
        {#if !setstore}
            <!-- show inputs for renaming -->
            <div class="ui_center ui_margin_top">
                <div class="ui_output">Store-name</div>
                <div class="ui_margin_top ui_input">
                    <input type="text" bind:value={newname} default={name}>
                </div>
                <button class="ui_button ui_margin_top" on:click={() => { setStore() }}>Update</button>
            </div>  
        {:else}
            <!-- show success message -->
            <div class="ui_text_large ui_center ui_margin_top">Successfully updated GROUP {id} FileInterface Store-name...</div>
