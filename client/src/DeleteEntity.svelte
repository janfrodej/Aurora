<!--
    ...
    Description: Delete an AURORA-entity.
-->
...
<script>
    ...
    async function deleteEntity() {
        ...
        for (let i=0; i < id.length; i++) {
            ...
            rmcount++;
            let mv=await call_aurora("delete"+nicetype,params);
            if (mv.err == 0) { removedcount++; }
        }
    }
...
{#if !disabled}
    {#if data != undefined}
        ...
        {/await}
    {/if}
    {#if removeent != undefined}
        {#await removeent}
            <Status message="Deleting {rmcount}/{id.length} entities..." type="processing" />
        {:then result}
            {setRemoved()}
        {/await}
    {/if}
    {#if show && id.length > 0}
        ...
        <div class="ui_title ui_center">Delete Entities</div>        
        {#if !removed}
            <!-- show delete mesage -->
            <div class="ui_center ui_text_large ui_margin_top">Delete the following {(id.length > 1 ? "entities" : "entity")}:</div>
            ...
            <div class="ui_center">                  
                ...
                <button class="ui_button" on:click={() => { removeent=deleteEntity() }}>Delete</button>
            </div>  
        {:else}
            <!-- show success message -->
            <div class="ui_text_large ui_center ui_margin_top">
                {#if removedcount > 0}
                    Successfully deleted {removedcount} of {id.length} entitites...
                {:else}
                    Unable to delete any of the entities. Please check error messages...
                {/if}    
            </div>
