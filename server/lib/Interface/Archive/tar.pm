...
#
# Interface::Archive::tar - Class for rendering tar-archives to AURORA datasets
#
package Interface::Archive::tar;
...
sub new {
   ...
   if (!exists $pars->{binary}) { $pars->{binary}="/bin/tar"; }
...
sub startArchiving {
   ...
   my $read=gensym;
   my $pid=open3(undef,$read,$read,$binary,"-czf",$destination,@{$paths});
   if (defined $pid) {
      # parent - do nada
   } else {
      # error comes in $!
      ...
      $self->{actime}=0;
      $self->{aread}=$read;
      print $write "Unable to run tar-archiving command: $!\n";
      chdir ($oldfolder);
      return;
   }
...
C<Interface::Archive::tar> - Class for generating tar-sets of AURORA dataset data.
...
It makes it possible to generate a tar-set of all or parts of the AURORA dataset data and then make a MIME URL as return value telling where the 
...
Inherits from the Archive-class and then sets the format ("tar.gz") and binary location for the tar-command (if no override by user 
has been specified).
